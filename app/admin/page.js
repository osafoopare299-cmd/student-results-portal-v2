...
export default function AdminPage() {
  ...
  return <main className="adminShell">
    ...
    <section className="adminCard"><div className="adminCardHead"><div><span className="eyebrow">STEP 3</span><h2>Publish, export or archive examination</h2></div><Eye size={20}/></div><div className="examList">{activeExam?<div className="examRow"><div><b>{activeExam.exam_name}</b><span>{activeExam.subject||'No subject'} · {activeExam.written_count} written · {activeExam.viva_count} viva · {activeExam.additional_count} additional</span></div><div className="examActions"><button className={activeExam.published?'publish on':'publish'} onClick={()=>save({action:'publish_exam',examId:activeExam.id,published:!activeExam.published})}>{activeExam.published?<><Eye size={16}/> Published</>:<><EyeOff size={16}/> Unpublished</>}</button>{activeExam.published&&<><button className="publish on" onClick={()=>window.location.href=`/api/admin/export?examId=${activeExam.id}`}><Download size={16}/> Download Excel</button><button className="publish on" onClick={()=>window.location.href=`/api/admin/export-pdf?examId=${activeExam.id}`}><FileText size={16}/> Download PDF</button></>}<button className="publish" onClick={async()=>{if(window.confirm(`Archive ${activeExam.exam_name}? Students will no longer see it, but its data will be preserved.`)) await save({action:'archive_exam',examId:activeExam.id})}}><Archive size={16}/> Archive</button></div></div>:<p className="muted">No active examination is available.</p>}</div></section>
