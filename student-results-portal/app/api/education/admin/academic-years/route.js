...
export async function POST(request){
  ...
  try{
    ...
    if(startsOn&&endsOn&&new Date(endsOn)<=new Date(startsOn)) return NextResponse.json({ok:false,error:'Academic year end date must be after the start date.'},{status:400});
    const sql=getEducationSql();
    ...
    await sql`insert into edu_audit_logs (action,entity_type,entity_id,metadata) values ('admin_academic_year_saved','edu_academic_year',${String(rows?.[0]?.id||'')},${JSON.stringify({name,startsOn,endsOn,isActive:active})}::jsonb)`;
