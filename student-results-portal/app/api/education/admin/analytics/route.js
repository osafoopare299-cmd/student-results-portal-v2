...
import { ensureAttendanceTables } from '../../../../../lib/education-attendance';
import { ensureResultReleaseTable } from '../../../../../lib/education-result-release';
...
export async function GET(){
  ...
  try{
    ...
    await Promise.all([ensureAttendanceTables(sql),ensureResultReleaseTable(sql)]);
    const [people,enrolments,materials,assessments,attempts,releases,attendance,recentActivity,coursePerformance]=await Promise.all([
      ...
      sql`select count(*) filter (where status='started')::int as started,count(*) filter (where status='submitted')::int as submitted,count(*) filter (where status='marked')::int as marked from edu_assessment_attempts`,
      sql`select count(*)::int as released,count(*) filter (where published_at>=now()-interval '30 days')::int as released_30d from edu_result_publications`,
      sql`select count(*) filter (where s.status='published')::int as sessions,count(r.id)::int as records,count(r.id) filter (where r.status in ('present','late'))::int as attended from edu_attendance_sessions s left join edu_attendance_records r on r.session_id=s.id`,
      sql`select action,entity_type,entity_id,created_at from edu_audit_logs order by created_at desc limit 15`,
      sql`select c.code,c.title,count(rp.id)::int as released_results,round(avg(case when a.max_score>0 then (t.score/a.max_score)*100 end)::numeric,2) as average_percentage from edu_result_publications rp join edu_assessment_attempts t on t.id=rp.attempt_id join edu_assessments a on a.id=t.assessment_id join edu_course_offerings o on o.id=a.offering_id join edu_courses c on c.id=o.course_id group by c.id,c.code,c.title order by released_results desc,c.code limit 12`
    ]);
    const p=people[0]||{},e=enrolments[0]||{},m=materials[0]||{},a=assessments[0]||{},t=attempts[0]||{},r=releases[0]||{},att=attendance[0]||{};
    const attendanceRate=n(att.records)>0?Math.round((n(att.attended)/n(att.records))*10000)/100:0;
    return NextResponse.json({ok:true,summary:{students:n(p.students),lecturers:n(p.lecturers),admins:n(p.admins),activeEnrolments:n(e.active),enrolledStudents:n(e.enrolled_students),activeOfferings:n(e.active_offerings),materials:n(m.total),publishedMaterials:n(m.published),aiApprovedMaterials:n(m.ai_approved),offlineMaterials:n(m.offline_ready),assessments:n(a.total),publishedAssessments:n(a.published),draftAssessments:n(a.drafts),closedAssessments:n(a.closed),startedAttempts:n(t.started),submittedAttempts:n(t.submitted),markedAttempts:n(t.marked),releasedResults:n(r.released),releasedLast30Days:n(r.released_30d),attendanceSessions:n(att.sessions),attendanceRecords:n(att.records),attendanceRate},coursePerformance,recentActivity});
  }catch(error){
