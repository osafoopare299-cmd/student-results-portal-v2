  ...
  try {
    ...
    const rows = await sql`
      select u.id, u.full_name, u.email, u.role, u.status,
             sp.student_number, c.name as class_name,
...
export async function POST(request) {
  ...
  try {
    ...
    if (!fullName || !email || !['student','lecturer','admin'].includes(role) || !['active','inactive','suspended'].includes(status)) {
      return NextResponse.json({ok:false,error:'Please provide a valid name, email, role and status.'},{status:400});
    }

    const sql = getSql();
    const rows = await sql`
      insert into edu_users (full_name,email,role,status)
      values (${fullName},${email},${role},${status})
