...
export async function GET(){
  ...
  try{
    ...
    const [offerings,assessmentStats,attemptStats,attendance,releasedRows,bands]=await Promise.all([
    ...
    const a=assessmentStats[0]||{},t=attemptStats[0]||{},att=attendance[0]||{};
    const activeStudents=new Set();offerings.forEach(o=>{if(n(o.students)>0)activeStudents.add(String(o.id));});
    const totalStudents=offerings.reduce((sum,o)=>sum+n(o.students),0);
    const attendanceRate=n(att.records)>0?Math.round((n(att.attended)/n(att.records))*10000)/100:0;
    return NextResponse.json({ok:true,summary:{offerings:offerings.length,enrolledStudents:totalStudents,assessments:n(a.total),draftAssessments:n(a.drafts),publishedAssessments:n(a.published),closedAssessments:n(a.closed),markingBacklog:n(t.submitted),marked:n(t.marked),released:n(t.released),attendanceSessions:n(att.sessions),attendanceRate},offerings,coursePerformance});
