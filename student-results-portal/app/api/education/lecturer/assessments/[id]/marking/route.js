...
async function claimVersion(sql,attemptId,expectedVersion){
  const rows=await sql`update edu_assessment_attempts set marking_version=marking_version+1 where id=${attemptId} and marking_version=${expectedVersion} returning marking_version`;
  return rows?.[0]?.marking_version;
}

function stale(){return NextResponse.json({ok:false,error:'This submission changed in another tab or session. Reload the marking workspace before continuing.',code:'STALE_MARKING'},{status:409});}
...
export async function GET(request,{params}){
  ...
  try{
    ...
    await ensureEducationResultReleaseSchema(sql);await ensureMarkingVersion(sql);
    ...
    const answers=await sql`select q.id as question_id,q.position,q.question_type,q.prompt,q.max_score,q.required,a.id as answer_id,a.answer,a.score,a.feedback,a.updated_at from edu_assessment_questions q left join edu_assessment_answers a on a.question_id=q.id and a.attempt_id=${attemptId} where q.assessment_id=${id} order by q.position`;
...
export async function POST(request,{params}){
  ...
  try{
    ...
    const assessment=await ownedAssessment(sql,id,access.user.id);
    if(!assessment)return NextResponse.json({ok:false,error:'Assessment not found.'},{status:404});
    const attemptId=Number(b.attemptId),expectedVersion=Number(b.markingVersion);
    ...
    if(!attempt)return NextResponse.json({ok:false,error:'Submission not found.'},{status:404});
    if(!Number.isInteger(expectedVersion)||expectedVersion!==Number(attempt.marking_version))return stale();
    if(b.action==='save-answer'){
      ...
      const nextVersion=await claimVersion(sql,attemptId,expectedVersion);if(nextVersion===undefined)return stale();
    ...
    if(b.action==='finalize'){
      ...
      if(total<0||total>Number(assessment.max_score))return NextResponse.json({ok:false,error:'Calculated score is outside the allowed assessment range.'},{status:400});
      const feedback=clean(b.feedback),nextVersion=await claimVersion(sql,attemptId,expectedVersion);if(nextVersion===undefined)return stale();
      ...
      return NextResponse.json({ok:true,status:'marked',score:total,released:false,markingVersion:nextVersion});
    }
    if(b.action==='release'){
      ...
      return NextResponse.json({ok:true,released:true,markingVersion:nextVersion});
    }
    if(b.action==='withdraw-release'){
      if(!attempt.released_at)return NextResponse.json({ok:true,released:false,markingVersion:expectedVersion});
      const nextVersion=await claimVersion(sql,attemptId,expectedVersion);if(nextVersion===undefined)return stale();
      ...
      return NextResponse.json({ok:true,released:false,markingVersion:nextVersion});
    }
