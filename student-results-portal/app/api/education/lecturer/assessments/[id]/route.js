...
export async function POST(request,{params}){
  ...
  try{const {id}=await params,b=await request.json(),sql=getEducationSql(),assessment=await ownedAssessment(sql,id,access.user.id);if(!assessment)return NextResponse.json({ok:false,error:'Assessment not found.'},{status:404});
    if(b.action==='publish'){const total=await sql`select coalesce(sum(max_score),0)::numeric as total,count(*)::int as count from edu_assessment_questions where assessment_id=${id}`;if(!total[0]?.count)return NextResponse.json({ok:false,error:'Add at least one question before publishing.'},{status:400});await sql`update edu_assessments set status='published',updated_at=now() where id=${id}`;return NextResponse.json({ok:true});}
    ...
    const pos=await sql`select coalesce(max(position),0)+1 as next from edu_assessment_questions where assessment_id=${id}`;const correct=type==='mcq'?JSON.stringify({value:clean(b.correctAnswer,1000)}):null;
