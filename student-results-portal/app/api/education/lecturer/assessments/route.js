...
export async function POST(request){
  ...
  try{
    ...
    const offeringId=b.offeringId,title=clean(b.title,240),description=clean(b.description,2000)||null,type=clean(b.assessmentType,20).toLowerCase();
    const instructions=clean(b.instructions,5000)||null,status=b.publish?'published':'draft';
    const maxScore=Number(b.maxScore),duration=b.durationMinutes?Number(b.durationMinutes):null;
    const opensAt=b.opensAt?new Date(b.opensAt).toISOString():null,closesAt=b.closesAt?new Date(b.closesAt).toISOString():null;
    if(!offeringId||!title||!['mcq','written','viva','osce','practical'].includes(type)||!Number.isFinite(maxScore)||maxScore<=0)return NextResponse.json({ok:false,error:'Course, title, valid assessment type and maximum score are required.'},{status:400});
    ...
    if(opensAt&&closesAt&&new Date(closesAt)<=new Date(opensAt))return NextResponse.json({ok:false,error:'Closing time must be after opening time.'},{status:400});
