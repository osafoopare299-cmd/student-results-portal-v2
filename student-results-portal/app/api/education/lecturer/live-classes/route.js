...
async function ownedClass(sql, classId, userId) {
  return (await sql`select l.*,c.code,c.title as course_title,cl.name as class_name from edu_live_classes l join edu_course_offerings o on o.id=l.offering_id join edu_courses c on c.id=o.course_id join edu_classes cl on cl.id=o.class_id where l.id=${classId} and o.lecturer_user_id=${userId} limit 1`)[0];
}

async function loadData(sql, userId) {
  const [offerings, classes] = await Promise.all([
    ...
    sql`select l.id,l.offering_id,l.title,l.starts_at,l.ends_at,l.status,c.code,c.title as course_title,cl.name as class_name,count(distinct a.user_id)::int as attendees,coalesce(sum(a.total_seconds),0)::int as total_seconds from edu_live_classes l join edu_course_offerings o on o.id=l.offering_id join edu_courses c on c.id=o.course_id join edu_classes cl on cl.id=o.class_id left join edu_live_attendance a on a.live_class_id=l.id where o.lecturer_user_id=${userId} group by l.id,c.code,c.title,cl.name order by l.starts_at desc limit 100`,
  ]);
  const breakouts = await sql`select b.id,b.live_class_id,b.name,b.daily_room_name,count(a.student_user_id)::int as assigned from edu_live_breakout_rooms b join edu_live_classes l on l.id=b.live_class_id join edu_course_offerings o on o.id=l.offering_id left join edu_live_breakout_assignments a on a.breakout_room_id=b.id where o.lecturer_user_id=${userId} group by b.id order by b.id`;
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ok:true,configured:dailyConfigured(),...(await loadData(sql,access.user.id))});
...
export async function POST(request) {
  ...
  try {
    ...
    if(body.action==='create'){
      ...
      const offering=(await sql`select o.id,c.code from edu_course_offerings o join edu_courses c on c.id=o.course_id where o.id=${offeringId} and o.lecturer_user_id=${access.user.id} limit 1`)[0];
      ...
      const rows=await sql`insert into edu_live_classes (offering_id,title,starts_at,ends_at,daily_room_name,created_by) values (${offeringId},${title},${startsAt.toISOString()},${endsAt.toISOString()},${daily.name},${access.user.id}) returning *`;
      return NextResponse.json({ok:true,liveClass:rows[0]});
    }
    if(body.action==='join'){
      const liveClass=await ownedClass(sql,Number(body.classId),access.user.id);
    ...
    if(body.action==='breakouts'){
      const liveClass=await ownedClass(sql,Number(body.classId),access.user.id),count=Math.min(8,Math.max(2,Number(body.count)||2));
      if(!liveClass)return NextResponse.json({ok:false,error:'Live class not found.'},{status:404});
