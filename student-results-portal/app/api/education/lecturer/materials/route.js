...
export async function POST(request){
 const access=await getEducationUser('lecturer'); if(!access.ok)return NextResponse.json({ok:false,error:'Lecturer access required.'},{status:401});
 try{const b=await request.json(),sql=getEducationSql();const offeringId=b.offeringId;const title=clean(b.title,240);const description=clean(b.description,2000)||null;const materialType=clean(b.materialType,20)||'note';const resourceUrl=clean(b.resourceUrl,1500)||null;const contentText=clean(b.contentText,20000)||null;
