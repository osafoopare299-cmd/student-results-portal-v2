...
export async function GET() {
  ...
  if (!auth) return NextResponse.json({ ok:false, error:'Education authentication is not configured yet.' }, { status:503 });
  ...
  try {
    ...
    const user = rows?.[0];
    if (!user) return NextResponse.json({ ok:false, error:'Your account is authenticated but is not enrolled in Dropare Education.' }, { status:403 });
    if (user.status !== 'active') return NextResponse.json({ ok:false, error:'Your education account is not active.' }, { status:403 });
