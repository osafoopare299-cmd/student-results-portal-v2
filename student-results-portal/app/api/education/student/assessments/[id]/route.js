...
export async function GET(request,{params}){
  ...
  try{const {id}=await params,sql=getEducationSql();await ensureEducationAssessmentReviewSchema(sql);const assessment=await studentAssessment(sql,id,access.user.id);if(!assessment)return NextResponse.json({ok:false,error:'Assessment not available.'},{status:404});
    ...
    return NextResponse.json({ok:true,assessment,questions,attempt:attempts[0]||null,answers,reviewLocked:false});
...
export async function POST(request,{params}){
  ...
  try{const {id}=await params,b=await request.json(),sql=getEducationSql();await ensureEducationAssessmentReviewSchema(sql);const assessment=await studentAssessment(sql,id,access.user.id);if(!assessment)return NextResponse.json({ok:false,error:'Assessment not available.'},{status:404});
    ...
    if(b.action==='submit'){
      const questions=await sql`select id,question_type,correct_answer,max_score,required from edu_assessment_questions where assessment_id=${id}`;const answers=await sql`select id,question_id,answer from edu_assessment_answers where attempt_id=${attempt.id}`;
      ...
      let autoScore=0;for(const q of questions){if(q.question_type!=='mcq')continue;const a=answers.find(x=>Number(x.question_id)===Number(q.id));const expected=q.correct_answer?.value??'';const actual=a?.answer?.value??'';const score=String(actual)===String(expected)?Number(q.max_score):0;autoScore+=score;if(a)await sql`update edu_assessment_answers set score=${score} where id=${a.id}`;}
