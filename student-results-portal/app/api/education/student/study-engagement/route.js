...
function streakFromDates(values){
  ...
  for(let i=1;i<unique.length;i++){
    const diff=(toDay(unique[i-1])-toDay(unique[i]))/dayMs;
    if(diff===1){run++;longest=Math.max(longest,run);}else run=1;
  }
  const today=new Date();const todayKey=today.toISOString().slice(0,10);const yesterdayKey=new Date(Date.now()-dayMs).toISOString().slice(0,10);
  let current=0;
...
export async function GET(){
  ...
  try{
    ...
    const [activity,counts,reminders,courses]=await Promise.all([
      sql`
        select activity_day from (
          select (completed_at at time zone 'Africa/Accra')::date::text as activity_day from edu_ai_practice_history where student_user_id=${access.user.id}
          union all
          select (last_reviewed_at at time zone 'Africa/Accra')::date::text from edu_student_flashcards where student_user_id=${access.user.id} and last_reviewed_at is not null
          union all
          select (completed_at at time zone 'Africa/Accra')::date::text from edu_student_study_tasks where student_user_id=${access.user.id} and completed_at is not null
        ) x where activity_day is not null order by activity_day desc
      `,
      sql`
        select
          (select count(*)::int from edu_ai_practice_history where student_user_id=${access.user.id}) as ai_sessions,
          (select count(*)::int from edu_student_flashcards where student_user_id=${access.user.id}) as flashcards_created,
          (select coalesce(sum(review_count),0)::int from edu_student_flashcards where student_user_id=${access.user.id}) as flashcard_reviews,
          (select count(*)::int from edu_student_flashcards where student_user_id=${access.user.id} and status='mastered') as flashcards_mastered,
          (select count(*)::int from edu_student_study_tasks where student_user_id=${access.user.id} and status='completed') as tasks_completed
      `,
      sql`
        select r.id,r.title,r.remind_at,r.repeat_rule,r.is_active,r.offering_id,c.code as course_code,c.title as course_title
        ...
        where r.student_user_id=${access.user.id}
        order by r.is_active desc,r.remind_at asc
        limit 100
      `,
      sql`
    ...
    const streak=streakFromDates(activity.map(x=>x.activity_day));
    const c=counts[0]||{};const total=Number(c.ai_sessions||0)+Number(c.flashcard_reviews||0)+Number(c.tasks_completed||0);
    const badges=[
      ...
      badge('practice-10','Practice Builder','Complete 10 AI Tutor practice sessions.',Number(c.ai_sessions)>=10),
      badge('cards-25','Card Collector','Create 25 private flashcards.',Number(c.flashcards_created)>=25),
      badge('review-100','Revision Engine','Complete 100 flashcard reviews.',Number(c.flashcard_reviews)>=100),
      badge('master-20','Memory Master','Master 20 flashcards.',Number(c.flashcards_mastered)>=20),
      badge('planner-20','Plan Finisher','Complete 20 study-planner tasks.',Number(c.tasks_completed)>=20)
    ];
    const points=Number(c.ai_sessions||0)*10+Number(c.flashcard_reviews||0)*2+Number(c.tasks_completed||0)*5+Number(c.flashcards_mastered||0)*5;
    const level=points>=1000?'Scholar':points>=500?'Achiever':points>=200?'Focused Learner':points>=50?'Rising Learner':'Starter';
    return NextResponse.json({ok:true,streak,badges,points,level,counts:c,reminders,courses});
