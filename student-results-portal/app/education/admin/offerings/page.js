...
export default function CourseOfferings(){
  ...
  const yearMismatch=Boolean(selectedClass?.academic_year_id&&form.academicYearId&&String(selectedClass.academic_year_id)!==String(form.academicYearId));

  async function save(e){
    e.preventDefault();
    if(yearMismatch){setMessage('Choose the academic year linked to the selected class.');return;}
  ...
  return <main style={{minHeight:'100vh',background:'#f5faf8',padding:'clamp(14px,3vw,24px)',fontFamily:'Arial,sans-serif',color:'#153126'}}>
    <section style={{maxWidth:1120,margin:'auto'}}>
      ...
      <h1 style={{fontSize:'clamp(28px,5vw,36px)',marginBottom:8}}>Course Offerings</h1>
      <p style={{color:'#657d74',maxWidth:760}}>Create the teaching instance that connects a course to a class, academic year, term and lecturer.</p>
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,320px),1fr))',gap:18,alignItems:'start'}}>
        <form onSubmit={save} style={{...card,display:'grid',gap:12,minWidth:0}}>
          ...
          <input required style={field} placeholder="Term 1 / Rotation 2" value={form.term} onChange={e=>setForm({...form,term:e.target.value})}/>
          ...
          {yearMismatch&&<small style={{color:'#9a5a12'}}>This class is linked to a different academic year. Select the class&apos;s academic year before saving.</small>}
          <button disabled={yearMismatch} style={{background:'#08744d',color:'#fff',border:0,borderRadius:12,padding:13,fontWeight:800,opacity:yearMismatch?.55:1}}>Save course offering</button>
