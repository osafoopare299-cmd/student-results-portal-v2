...
import { getAdminDashboard } from '../../../lib/education-dashboard';
...
export default async function EducationAdminPortal(){
  ...
  if (!authenticated) {
    return <main style={{minHeight:'100vh',display:'grid',placeItems:'center',padding:'24px',background:'linear-gradient(160deg,#e9f8ef,#f7fbf8)',fontFamily:'Arial,sans-serif'}}>
      <section style={{width:'min(100%,460px)',background:'#fff',border:'1px solid #dcebe1',borderRadius:'24px',padding:'32px',boxShadow:'0 22px 60px rgba(17,78,45,.12)'}}>
        ...
        <p style={{margin:'0 0 24px',lineHeight:1.65,color:'#64748b'}}>The Education Administrator workspace uses the existing secure administrator session. Sign in through the current administrator portal, then return here.</p>
  ...
  try {
    ...
    console.error('Education administrator dashboard unavailable:', error);
  }

  return <Workspace role="admin" dashboard={dashboard}/>;
}
