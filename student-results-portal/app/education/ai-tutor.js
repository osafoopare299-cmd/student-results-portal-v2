...
import { ArrowLeft, BookOpen, Bot, CheckCircle2, Clock3, FileQuestion, FileText, History, ListChecks, RotateCcw, Search, Send, ShieldCheck, Sparkles, Target } from 'lucide-react';
...
export default function AITutor(){
  ...
  const weakTopics=useMemo(()=>{const grouped=new Map();history.filter(item=>item.mode==='mcq'&&item.percent_score!==null&&item.percent_score!==undefined).forEach(item=>{const topicName=String(item.topic||'').trim();const key=topicName.toLowerCase().replace(/\s+/g,' ');if(!key)return;const current=grouped.get(key)||{topic:topicName,total:0,attempts:0,lastAt:0};current.total+=Number(item.percent_score)||0;current.attempts+=1;current.lastAt=Math.max(current.lastAt,new Date(item.completed_at).getTime()||0);grouped.set(key,current);});return [...grouped.values()].map(item=>({...item,average:item.total/item.attempts})).sort((a,b)=>a.average-b.average||b.lastAt-a.lastAt).slice(0,3);},[history]);
  ...
  return <main className={styles.page}>
    <header className={styles.header}><div><Link href="/education/student" className={styles.back}><ArrowLeft size={18}/> Student portal</Link><span className={styles.eyebrow}>APPROVED-MATERIAL AI</span><h1><Sparkles size={28}/> AI Tutor</h1><p>Learn, test yourself, and retry mistakes until they are corrected.</p></div><div className={styles.guard}><ShieldCheck size={20}/><span>Grounded answers only</span></div></header>
    <section className={styles.grid}>
      <aside className={styles.sources}>
        ...
        <div style={{marginTop:16,padding:14,border:'1px solid #cfe1d8',borderRadius:16,background:'#f8fcfa'}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}><Target size={18}/><b>Adaptive practice</b></div>{weakTopics.length?<><small style={{display:'block',color:'#60786e',lineHeight:1.45,marginBottom:9}}>Based on completed MCQ scores. Lowest-scoring topics are prioritised using your selected difficulty.</small><div style={{display:'grid',gap:7,marginBottom:10}}>{weakTopics.map((item,index)=><div key={item.topic} style={{display:'flex',justifyContent:'space-between',gap:8,padding:'8px 9px',borderRadius:10,background:'#fff'}}><div><b style={{fontSize:13}}>{index===0?'Focus first: ':''}{item.topic}</b><small style={{display:'block',marginTop:2,color:'#789087'}}>{item.attempts} attempt{item.attempts===1?'':'s'}</small></div><b style={{fontSize:13,color:item.average<70?'#b42318':'#08744d'}}>{Math.round(item.average)}%</b></div>)}</div><button disabled={loading||!sources.length} onClick={practiceWeakest} style={{...buttonStyle,width:'100%',justifyContent:'center',background:'#08744d',color:'#fff'}}><Target size={15}/> Practice my weak areas</button></>:<small style={{color:'#70857c',lineHeight:1.45}}>Complete at least one MCQ practice set and your weakest topics will appear here automatically.</small>}</div>
