...
function PracticeSet({set,onComplete}){
  ...
  function finish(reason='submitted'){
    ...
    const correct=set.mode==='mcq'?set.questions.reduce((sum,q,index)=>sum+(selected[index]===q.correctIndex?1:0),0):null;
    const elapsedSeconds=Math.max(0,Math.round((Date.now()-startedAt.current)/1000));
    onComplete?.({mode:set.mode,topic:set.topic,questionCount:set.questions.length,answeredCount:answered,correctCount:correct,timed:Boolean(set.timed),durationMinutes:set.timed?set.durationMinutes:null,elapsedSeconds,completionReason:reason,offeringId:set.offeringId||null,difficulty:set.difficulty||'standard'});
  }
  ...
  return <div style={{marginTop:14,padding:14,border:'1px solid #dbe9e2',borderRadius:16,background:'#fbfefd'}}>
    <div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center',marginBottom:10,flexWrap:'wrap'}}><div><b>{set.title}</b><small style={{display:'block',marginTop:3}}>{set.questions.length} questions • {difficultyLabel(set.difficulty)}{set.timed?` • ${set.durationMinutes} minute timed session`:' • self-paced'}</small></div>{set.timed&&<div style={{display:'flex',alignItems:'center',gap:7,padding:'8px 11px',borderRadius:12,background:finished?'#eef5f1':secondsLeft<=60?'#fff1f0':'#eef8f3',fontWeight:900,color:secondsLeft<=60&&!finished?'#b42318':'#08744d'}}><Clock3 size={17}/>{finished?'Finished':formatTime(secondsLeft)}</div>}</div>
    <div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center',padding:'10px 12px',borderRadius:12,background:'#f3f8f5',marginBottom:12,flexWrap:'wrap'}}><span style={{fontSize:13,fontWeight:700}}>{finished?`Answered ${answered} of ${set.questions.length}`:`${set.timed?'Countdown active':'Practice in progress'} • ${answered} answered`}</span>{!finished&&<button onClick={()=>{if(!set.timed||window.confirm('Submit this practice now?'))finish('submitted');}} style={{border:0,background:'#08744d',color:'#fff',padding:'8px 11px',borderRadius:10,fontWeight:800}}>{set.timed?'Submit practice':'Finish practice'}</button>}{finished&&set.mode==='mcq'&&<b>Score: {score}/{set.questions.length} ({Math.round((score/set.questions.length)*100)}%)</b>}{finished&&set.mode==='written'&&<b>Completed</b>}</div>
    <div style={{display:'grid',gap:12}}>{set.questions.map((q,index)=><article key={q.id||index} style={{background:'#fff',border:'1px solid #e3eee8',borderRadius:14,padding:13}}><b>{index+1}. {q.question}</b>
      {set.mode==='mcq'?<div style={{display:'grid',gap:7,marginTop:10}}>{(q.options||[]).map((opt,i)=>{const chosen=selected[index]===i,reveal=finished||(!set.timed&&shown[index]);return <button key={i} disabled={finished} onClick={()=>choose(index,i)} style={{textAlign:'left',padding:'10px 11px',borderRadius:10,border:'1px solid #d9e7e0',background:reveal&&i===q.correctIndex?'#e9f8ef':reveal&&chosen&&i!==q.correctIndex?'#fff1f0':chosen?'#eef8f3':'#fff',fontWeight:chosen?800:600,cursor:finished?'default':'pointer'}}>{String.fromCharCode(65+i)}. {opt}</button>})}{(finished||(!set.timed&&shown[index]))&&<div style={{padding:10,borderRadius:10,background:'#f4faf7',lineHeight:1.5}}><b>{selected[index]===q.correctIndex?'Correct.':'Review this answer.'}</b> {q.explanation}</div>}</div>:<div style={{marginTop:10}}>{!finished&&<textarea value={writtenAnswers[index]||''} onChange={e=>setWrittenAnswers(value=>({...value,[index]:e.target.value}))} placeholder="Type your answer here…" rows={4} style={{width:'100%',boxSizing:'border-box',border:'1px solid #d6e4dd',borderRadius:10,padding:10,resize:'vertical'}}/>}{finished&&<><button onClick={()=>setShown(v=>({...v,[index]:!v[index]}))} style={{padding:'9px 11px',borderRadius:10,border:'1px solid #cfe1d8',background:'#eef8f3',fontWeight:800,color:'#08744d'}}>{shown[index]?'Hide answer guide':'Show answer guide'}</button>{shown[index]&&<div style={{marginTop:8,padding:10,borderRadius:10,background:'#f4faf7',lineHeight:1.55}}>{q.answerGuide}</div>}</>}</div>}
    </article>)}</div>
  </div>;
}
...
export default function AITutor(){
  ...
  async function loadHistory(){
    try{const r=await fetch('/api/education/student/ai-tutor/history',{cache:'no-store'});const d=await r.json();if(r.ok&&d.ok)setHistory(d.history||[]);}catch{}
  }

  useEffect(()=>{
    fetch('/api/education/student/ai-tutor',{cache:'no-store'}).then(r=>r.json()).then(data=>{if(data.ok)setSources(data.sources||[]);else setError(data.error||'AI Tutor sources unavailable.');}).catch(()=>setError('AI Tutor sources unavailable.'));
    loadHistory();
  },[]);
  ...
  async function savePractice(payload){
    try{
      const response=await fetch('/api/education/student/ai-tutor/history',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});
      const data=await response.json();
      if(!response.ok||!data.ok)throw new Error(data.error||'Unable to save practice history.');
      setHistoryMsg('Practice saved to your history.');
      await loadHistory();
    }catch(e){setHistoryMsg(e.message||'Unable to save practice history.');}
  }
  ...
  async function generatePractice(type,focus,label,scopeOverride){
    ...
    const offeringId=scopeOverride===undefined?(course==='all'?null:Number(course)):scopeOverride;
    try{const data=await post({mode:type,topic:clean,count,difficulty},scopeOverride);setMessages(prev=>[...prev,{kind:'practice',q:label||`${type==='mcq'?'MCQs':'Written questions'} on: ${clean}`,set:{mode:type,title:data.title||'Practice questions',questions:data.questions||[],timed,durationMinutes:timed?durationMinutes:null,topic:clean,offeringId,difficulty:data.difficulty||difficulty},a:data.answer||'',citations:data.citations||[],serviceReady:data.serviceReady!==false}]);}
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.grid}>
      <aside className={styles.sources}><div className={styles.sectionTitle}><BookOpen size={19}/><div><b>Approved sources</b><small>{visibleSources.length} available</small></div></div>
        ...
        <div style={{marginTop:16,padding:14,border:'1px solid #cfe1d8',borderRadius:16,background:'#f8fcfa'}}><div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}><Target size={18}/><b>Adaptive practice</b></div>{weakTopics.length?<><small style={{display:'block',color:'#60786e',lineHeight:1.45,marginBottom:9}}>Based on your completed MCQ scores. Lowest-scoring topics are prioritised using your currently selected difficulty.</small><div style={{display:'grid',gap:7,marginBottom:10}}>{weakTopics.map((item,index)=><div key={item.topic} style={{display:'flex',justifyContent:'space-between',gap:8,padding:'8px 9px',borderRadius:10,background:'#fff'}}><div><b style={{fontSize:13}}>{index===0?'Focus first: ':''}{item.topic}</b><small style={{display:'block',marginTop:2,color:'#789087'}}>{item.attempts} attempt{item.attempts===1?'':'s'}</small></div><b style={{fontSize:13,color:item.average<70?'#b42318':'#08744d'}}>{Math.round(item.average)}%</b></div>)}</div><button disabled={loading||!sources.length} onClick={practiceWeakest} style={{...buttonStyle,width:'100%',justifyContent:'center',background:'#08744d',color:'#fff'}}><Target size={15}/> Practice my weak areas</button></>:<small style={{color:'#70857c',lineHeight:1.45}}>Complete at least one MCQ practice set and your weakest topics will appear here automatically.</small>}</div>
