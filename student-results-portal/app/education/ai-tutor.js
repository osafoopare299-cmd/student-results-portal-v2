...
function PracticeSet({set,onComplete}){
  ...
  function finish(reason='submitted'){
    ...
    onComplete?.({mode:set.mode,topic:set.topic,questionCount:set.questions.length,answeredCount:answered,correctCount:correct,timed:Boolean(set.timed),durationMinutes:set.timed?set.durationMinutes:null,elapsedSeconds,completionReason:reason,offeringId:set.offeringId||null});
  }
...
export default function AITutor(){
  ...
  const [topic,setTopic]=useState(''),[practiceType,setPracticeType]=useState('mcq'),[count,setCount]=useState(5),[timed,setTimed]=useState(false),[durationMinutes,setDurationMinutes]=useState(20),[history,setHistory]=useState([]),[historyMsg,setHistoryMsg]=useState('');
