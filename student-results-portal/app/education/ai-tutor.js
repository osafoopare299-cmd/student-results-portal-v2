...
import { ArrowLeft, BookOpen, Bot, CheckCircle2, FileQuestion, FileText, ListChecks, Search, Send, ShieldCheck, Sparkles } from 'lucide-react';
...
function PracticeSet({set}){
  ...
  return <div style={{marginTop:14,padding:14,border:'1px solid #dbe9e2',borderRadius:16,background:'#fbfefd'}}>
    <div style={{display:'flex',justifyContent:'space-between',gap:8,alignItems:'center',marginBottom:10}}><b>{set.title}</b><small>{set.questions.length} questions</small></div>
    <div style={{display:'grid',gap:12}}>{set.questions.map((q,index)=><article key={q.id||index} style={{background:'#fff',border:'1px solid #e3eee8',borderRadius:14,padding:13}}><b>{index+1}. {q.question}</b>
      {set.mode==='mcq'?<div style={{display:'grid',gap:7,marginTop:10}}>{(q.options||[]).map((opt,i)=>{const chosen=selected[index]===i,reveal=shown[index];return <button key={i} onClick={()=>{setSelected(v=>({...v,[index]:i}));setShown(v=>({...v,[index]:true}));}} style={{textAlign:'left',padding:'10px 11px',borderRadius:10,border:'1px solid #d9e7e0',background:reveal&&i===q.correctIndex?'#e9f8ef':chosen?'#f6f8f7':'#fff',fontWeight:chosen?800:600}}>{String.fromCharCode(65+i)}. {opt}</button>})}{shown[index]&&<div style={{padding:10,borderRadius:10,background:'#f4faf7',lineHeight:1.5}}><b>{selected[index]===q.correctIndex?'Correct.':'Review this answer.'}</b> {q.explanation}</div>}</div>:<div style={{marginTop:10}}><button onClick={()=>setShown(v=>({...v,[index]:!v[index]}))} style={{padding:'9px 11px',borderRadius:10,border:'1px solid #cfe1d8',background:'#eef8f3',fontWeight:800,color:'#08744d'}}>{shown[index]?'Hide answer guide':'Show answer guide'}</button>{shown[index]&&<div style={{marginTop:8,padding:10,borderRadius:10,background:'#f4faf7',lineHeight:1.55}}>{q.answerGuide}</div>}</div>}
    </article>)}</div>
  </div>;
}
...
export default function AITutor(){
  ...
  const [topic,setTopic]=useState(''),[practiceType,setPracticeType]=useState('mcq'),[count,setCount]=useState(5);
  ...
  async function generatePractice(type,focus,label){
    ...
    try{const data=await post({mode:type,topic:clean,count});setMessages(prev=>[...prev,{kind:'practice',q:label||`${type==='mcq'?'MCQs':'Written questions'} on: ${clean}`,set:{mode:type,title:data.title||'Practice questions',questions:data.questions||[]},a:data.answer||'',citations:data.citations||[],serviceReady:data.serviceReady!==false}]);}
