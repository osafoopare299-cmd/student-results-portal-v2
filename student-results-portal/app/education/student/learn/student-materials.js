...
export default function StudentMaterials(){
  ...
  return <main style={{minHeight:'100vh',background:'#f4faf7',padding:24,fontFamily:'Arial,sans-serif',color:'#17342a'}}><div style={{maxWidth:1120,margin:'auto'}}>
    ...
    <section style={{display:'grid',gap:12}}>{materials.length?materials.map(m=>{const Icon=iconFor[m.material_type]||FileText;const saved=savedIds.includes(String(m.id));const href=resourceHref(m);return <article key={m.id} style={card}><div style={{display:'flex',gap:13,alignItems:'flex-start'}}><span style={{width:42,height:42,borderRadius:12,background:'#e8f7f0',display:'grid',placeItems:'center',color:'#08744d',flex:'none'}}><Icon size={20}/></span><div style={{flex:1,minWidth:0}}><div style={{display:'flex',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}><div><small style={{fontWeight:800,color:'#08744d'}}>{m.code} • {m.material_type.toUpperCase()}</small><h2 style={{fontSize:18,margin:'4px 0 6px'}}>{m.title}</h2></div><div style={{display:'flex',gap:7,alignItems:'center',flexWrap:'wrap'}}>{m.has_uploaded_file&&<span style={{fontSize:11,fontWeight:800,background:'#e8f7f0',padding:'6px 8px',borderRadius:999}}>Secure file</span>}{m.is_offline_available&&<span style={{fontSize:11,fontWeight:800,background:'#eef7f3',padding:'6px 8px',borderRadius:999}}>Offline allowed</span>}{saved&&<span style={{fontSize:11,fontWeight:800,background:'#e5f8ee',padding:'6px 8px',borderRadius:999}}><CheckCircle2 size={12} style={{verticalAlign:'middle'}}/> Saved</span>}{m.is_ai_approved&&<span style={{fontSize:11,fontWeight:800,background:'#eef7f3',padding:'6px 8px',borderRadius:999}}><Sparkles size={12} style={{verticalAlign:'middle'}}/> AI source</span>}</div></div>{m.description&&<p style={{color:'#60786e',lineHeight:1.55}}>{m.description}</p>}{m.content_text&&m.material_type==='note'&&<div style={{background:'#f7fbf9',borderRadius:12,padding:13,lineHeight:1.6,whiteSpace:'pre-wrap'}}>{m.content_text}</div>}{m.original_filename&&<div style={{fontSize:12,color:'#60786e',marginTop:8}}>{m.original_filename}{m.file_size_bytes?` • ${(Number(m.file_size_bytes)/1048576).toFixed(1)} MB`:''}</div>}<div style={{fontSize:12,color:'#789087',marginTop:10}}>{m.class_name}{m.term?` • ${m.term}`:''}{m.lecturer_name?` • ${m.lecturer_name}`:''}</div><div style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:12}}>{href&&<a href={href} target="_blank" rel="noreferrer" style={{display:'inline-flex',alignItems:'center',gap:6,color:'#08744d',fontWeight:800,textDecoration:'none'}}>Open resource <ExternalLink size={15}/></a>}{m.is_offline_available&&(saved?<button onClick={()=>removeOffline(m)} style={{border:0,background:'#eef5f1',color:'#526a60',borderRadius:10,padding:'8px 10px',fontWeight:800}}>Remove offline copy</button>:<button onClick={()=>saveOffline(m)} style={{border:0,background:'#08744d',color:'#fff',borderRadius:10,padding:'8px 10px',fontWeight:800,display:'inline-flex',alignItems:'center',gap:6}}><Download size={14}/> Save for offline</button>)}</div></div></div></article>;}):<section style={{...card,textAlign:'center',color:'#6b8077'}}>No materials match this view yet.</section>}</section>
