...
export async function sendEducationUpdateEmails({ recipients, title, message, kind = "Platform update", course = "", actionPath = "/education", eventId = "update" }) {
  ...
  for (let index = 0; index < people.length; index += 8) {
    ...
    const settled = await Promise.allSettled(batch.map((person) => sendOne({
      ...
      html: `<div style="background:#eef7f2;padding:32px 16px;font-family:Arial,sans-serif;color:#153c30"><div style="box-sizing:border-box;width:100%;max-width:600px;margin:auto;background:#fff;border:1px solid #d8e8e0;border-radius:16px;padding:32px"><p style="color:#08744d;font-size:13px;font-weight:700;letter-spacing:.08em">DROPARE EDUCATION · ${escapeHtml(kind.toUpperCase())}</p><h1 style="font-size:25px;line-height:1.25">${escapeHtml(title)}</h1><p>Hello ${escapeHtml(person.full_name || "there")},</p><p>${escapeHtml(message)}</p><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;margin:28px 0"><tr><td align="center" bgcolor="#08744d" style="border-radius:9px"><a href="${escapeHtml(url)}" style="box-sizing:border-box;display:block;width:100%;color:#fff;text-decoration:none;padding:14px 16px;border-radius:9px;font-weight:700;line-height:1.3;text-align:center">Open Education platform</a></td></tr></table><p style="color:#667b72;font-size:13px;line-height:1.5">This automatic email was sent because this update applies to your Education account.</p></div></div>`,
    })));
